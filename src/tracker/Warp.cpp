...
double FACETRACKER::pythag(double a, double b)
{
  ...
  if (absa > absb)
    ...
    return absb * std::sqrt(1.0 + SQR(absa / absb));
}
//=============================================================================
...
double FACETRACKER::bilinInterp(cv::Mat &I, const double x, const double y)
{
  ...
  {
    if (y1 == y2)
      return (double)I.at<uchar>(y1, x1);
    else
      return (double(y2) - y) * I.at<uchar>(y1, x1) +
             (y - double(y1)) * I.at<uchar>(y2, x1);
  }
  else
  {
    if (y1 == y2)
      return (double(x2) - x) * I.at<uchar>(y1, x1) +
...
void PAW::Write(ofstream &s, bool binary)
{
  ...
  else
  {
    ...
    IOBinary::WriteMat(s, _src);
    IOBinary::WriteMat(s, _tri);
    IOBinary::WriteMat(s, _tridx);
    IOBinary::WriteMat(s, _mask);
    IOBinary::WriteMat(s, _alpha);
    IOBinary::WriteMat(s, _beta);
  }
  return;
}
//===========================================================================
void PAW::ReadBinary(ifstream &s, bool readType)
{
  ...
  IOBinary::ReadMat(s, _src);
  IOBinary::ReadMat(s, _tri);
  IOBinary::ReadMat(s, _tridx);
  IOBinary::ReadMat(s, _mask);
  IOBinary::ReadMat(s, _alpha);
  IOBinary::ReadMat(s, _beta);
...
void PAW::Vectorize(cv::Mat &img, cv::Mat &vec)
{
  ...
  if ((vec.rows != _nPix) || (vec.cols != 1))
    vec.create(_nPix, 1, CV_64F);
  int i, j;
  ...
  default:
    printf("Unsupported image type %d!", img.type());
    abort();
  }
  return;
...
{
  if ((so.rows != si.rows) || (so.cols != si.cols))
    so.create(si.rows, 1, CV_64F);
  for (int i = 0; i < si.rows / 2; i++)
  {
                               ...
                               so.db(i, 0), so.db(i + so.rows / 2, 0)) < 0)
    {
      ...
      {
        if (idx->rows != si.rows / 2)
          return -1;
        so.db(i, 0) = 0.0;
        so.db(i + so.rows / 2, 0) = 0.0;
        int k = idx->it(i, 0), l = 0;
        double vx, vy;
...
int PAW::InverseWarpPoint(double xi, double yi, double &xo, double &yo, int t)
{
  ...
  {
    j = _tri.it(l, i);
    sx[i] = _src.db(j, 0) - _xmin;
    sy[i] = _src.db(j + n, 0) - _ymin;
    dx[i] = _dst.db(j, 0);
    dy[i] = _dst.db(j + n, 0);
    X.db(i, 0) = dx[i];
    X.db(i, 1) = dy[i];
  ...
  yo = A.db(1, 0) * xi + A.db(1, 1) * yi + A.db(1, 2);
  return 0;
}
//===========================================================================
...
{
  int i, j;
  cv::Mat vTri = cv::Mat::zeros(this->nPoints(), this->nTri(), CV_8U);
...
cv::Mat PAW::PixTri()
{
  ...
  for (i = 0; i < _h; i++)
  {
    ...
    {
      if (*mp)
        *pp++ = isWithinTri(double(j) + _xmin, double(i) + _ymin, _tri, _src);
    }
  }
  return pixTri;
}
...
{
  int x, y, l, k = -1;
  double vx, vy, a1 = 0, a2 = 0, a3 = 0, b1 = 0, b2 = 0, b3 = 0;
...
cv::Mat PAW::dWdx()
{
  cv::Mat pixTri = this->PixTri();
  return this->dWdx(pixTri);
}
//==============================================================================
