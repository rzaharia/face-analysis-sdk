...
using namespace std;
//=============================================================================
int myAvatar::numberOfAvatars() const
{
  return int(_images.size());
}
//=============================================================================
void myAvatar::setAvatar(int idx)
{
...
myAvatar::Thumbnail(int idx)
{
  if ((idx < 0) || (idx >= int(_images.size())))
    return cv::Mat();
  cv::Mat thumb = _images[idx].clone();
  return thumb;
}
//=============================================================================
void myAvatar::Read(ifstream &s, bool readType)
{
  if (readType)
  {
    int type;
    s >> type;
    assert(type == AVATAR::IO::MYAVATAR);
  }
  int k;
  s >> k;
  _images.resize(k);
  ...
  for (unsigned i = 0; i < _images.size(); i++)
  {
    ...
    s >> _scale[i];
    s >> _lpupil[i].rad >> _rpupil[i].rad >> _lpupil[i].px >> _rpupil[i].px >> _lpupil[i].py >> _rpupil[i].py >> _lpupil[i].scelera.val[0] >> _rpupil[i].scelera.val[0] >> _lpupil[i].scelera.val[1] >> _rpupil[i].scelera.val[1] >> _lpupil[i].scelera.val[2] >> _rpupil[i].scelera.val[2];
    ...
    _lpupil[i].image.resize(3);
    _rpupil[i].image.resize(3);
    for (int j = 0; j < 3; j++)
    {
      cv::Mat limg;
      FACETRACKER::IO::ReadMat(s, limg);
      cv::Mat rimg;
      FACETRACKER::IO::ReadMat(s, rimg);
  ...
  _expr.resize(k);
  for (int i = 0; i < k; i++)
    FACETRACKER::IO::ReadMat(s, _expr[i]);
  ...
  if (_lpupil[0].idx.rows > _rpupil[0].idx.rows)
    ...
    epts_.create(2 * _rpupil[0].idx.rows, 1, CV_64F);
  _idx = 0;
...
void myAvatar::ReadBinary(ifstream &s, bool readType)
{
  ...
  for (unsigned i = 0; i < _images.size(); i++)
  {
    ...
    {
      cv::Mat img;
      FACETRACKER::IOBinary::ReadMat(s, img);
      img.convertTo(rgb[j], CV_8U);
    }
    _images[i].create(rgb[0].rows, rgb[0].cols, CV_8UC3);
    cv::merge(rgb, _images[i]);
    ...
    {
      cv::Mat limg;
      FACETRACKER::IOBinary::ReadMat(s, limg);
      cv::Mat rimg;
      FACETRACKER::IOBinary::ReadMat(s, rimg);
      limg.convertTo(_lpupil[i].image[j], CV_8U);
      rimg.convertTo(_rpupil[i].image[j], CV_8U);
    }
...
void myAvatar::Write(ofstream &s, bool binary)
{
  if (!binary)
  {
    ...
    {
      vector<cv::Mat> rgb(3);
      cv::split(_images[i], rgb);
      for (int j = 0; j < 3; j++)
      {
        cv::Mat img;
        rgb[j].convertTo(img, CV_32S);
      ...
      {
        cv::Mat limg;
        _lpupil[i].image[j].convertTo(limg, CV_32S);
        cv::Mat rimg;
        _rpupil[i].image[j].convertTo(rimg, CV_32S);
  ...
  else
  {
    ...
    _warp.Write(s, binary);
    _pdm.Write(s, binary);
    _gpdm.Write(s, binary);
    _gen.Write(s, binary);
    for (unsigned i = 0; i < _images.size(); i++)
...
//=============================================================================
int myAvatar::Animate(cv::Mat &draw,
                      ...
                      void *params)
{
  ...
  {
    cv::Point lp, rp;
    double wl, wr, hl, hr, ldx, ldy, rdx, rdy, lrad, rrad, dx, dy;
    ...
    ldx = double(lp.x - _lp0.x) / wl;
    ldy = double(lp.y - _lp0.y) / hl;
    rdx = double(rp.x - _rp0.x) / wr;
    rdy = double(rp.y - _rp0.y) / hr;
    double ealpha = 0.3;
    ...
    lp.x = _lpupil[_idx].px + dx * wl;
    lp.y = _lpupil[_idx].py + dy * hl;
    rp.x = _rpupil[_idx].px + dx * wr;
    rp.y = _rpupil[_idx].py + dy * hr;
                       ...
                       int scale_eyes)
{
  ...
  if ((plocal_.rows != _pdm.nModes()) || (plocal_.cols != 1))
    plocal_.create(_pdm.nModes(), 1, CV_64F);
  if ((pglobl_.rows != 6) || (pglobl_.cols != 1))
    pglobl_.create(6, 1, CV_64F);
                           ...
                           cv::Mat &tri)
{
  ...
  //check points are within image
  {
    ...
    for (int i = 0; i < n; i++)
    {
      if ((src_pts.db(i, 0) < 0) || (src_pts.db(i, 0) >= cols) ||
          ...
          (dst_pts.db(i + n, 0) < 0) || (dst_pts.db(i + n, 0) >= rows))
        return;
    }
  }
  //normalize oral cavity colors
  {
    ...
    for (int i = 1; i < n; i++)
    {
      if (xmin > src_pts.db(i, 0))
        xmin = src_pts.db(i, 0);
      if (xmax < src_pts.db(i, 0))
        xmax = src_pts.db(i, 0);
      if (ymin > src_pts.db(n + i, 0))
        ymin = src_pts.db(n + i, 0);
      if (ymax < src_pts.db(n + i, 0))
        ymax = src_pts.db(n + i, 0);
    }
...
#pragma omp parallel for
  ...
  for (l = 0; l < tri.rows; l++)
  {
    ...
    for (i = 0; i < 3; i++)
    {
      j = tri.it(l, i);
      sx[i] = src_pts.db(j, 0);
      sy[i] = src_pts.db(j + n, 0);
      dx[i] = dst_pts.db(j, 0);
      dy[i] = dst_pts.db(j + n, 0);
    }
    ...
    if ((xmin < 0) || (xmax >= dst_img[0].cols) ||
        ...
        (((dx[1] - dx[0]) * (dy[0] - dy[2]) + (dy[1] - dy[0]) * (dx[2] - dx[0])) /
             ((sx[1] - sx[0]) * (sy[0] - sy[2]) + (sy[1] - sy[0]) * (sx[2] - sx[0])) <=
         0))
    {
      continue;
    }
...
{
  cv::Mat p;
  this->GetIdxPts(shape, idx, p, false);
...
//=============================================================================
void myAvatar::Initialise(const cv::Mat_<cv::Vec<uint8_t, 3>> &image,
                          const std::vector<cv::Point_<double>> &points,
                          void *params)
{
  cv::Mat shape = vectorise_points(points);
  ...
  else
  {
    ...
    for (unsigned i = 0; i < X.size(); i++)
    {
      ...
      _pdm.CalcParams(x, plocal_, pglobl_);
      plocal_.copyTo(y);
    }
    if (p->alpha == 1)
    { //use semantic correspondences only
      //learn user's basis
      cv::Mat user_basis;
      {
        cv::SVD svd(Y);
    ...
    else
    {
      ...
      for (unsigned i = 0; i < _shapes.size(); i++)
      {
        ...
        {
          cv::Mat g = Y * _expr[i].row(j).t();
          g.db(j, 0) += val;
          cv::Mat h = Hi * g;
          cv::Mat ht = h.t();
          cv::Mat r = _reg[i].row(j);
          ht.copyTo(r);
        }
      }
    }
...
//=============================================================================
void myAvatar::GetPupils(cv::Mat &im,
                         cv::Mat &shape,
                         cv::Point &pl,
                         cv::Point &pr)
{
                   ...
                   cv::Mat &tri)
{
  ...
  cv::Rect rect = this->GetBoundingBox(pt);
  double vx = 0, vy = 0, sum = 0;
  ...
  cv::Mat imn = im(rect).clone();
  equalizeHist(imn, imn);
...
//==============================================================================
void myAvatar::DrawPupil(double px,
                         double py,
                         double rad,
                         cv::Mat &pt,
                         vector<cv::Mat> &pupil,
                            ...
                            cv::Mat &ref)
{
  ...
  X.db(0, 0) = shape.db(36, 0);
  X.db(0, 1) = shape.db(36 + n, 0);
  X.db(0, 2) = 1;
  X.db(0, 3) = 0;
  X.db(1, 0) = shape.db(36 + n, 0);
  X.db(1, 1) = -shape.db(36, 0);
  X.db(1, 2) = 0;
  X.db(1, 3) = 1;
  X.db(2, 0) = shape.db(39, 0);
  X.db(2, 1) = shape.db(39 + n, 0);
  X.db(2, 2) = 1;
  X.db(2, 3) = 0;
  X.db(3, 0) = shape.db(39 + n, 0);
  X.db(3, 1) = -shape.db(39, 0);
  X.db(3, 2) = 0;
  X.db(3, 3) = 1;
  Y.db(0, 0) = ref.db(36, 0);
  Y.db(1, 0) = ref.db(36 + n, 0);
  Y.db(2, 0) = ref.db(39, 0);
  Y.db(3, 0) = ref.db(39 + n, 0);
                             ...
                             cv::Mat &ref)
{
  int n = _gpdm.nPoints();
  cv::Mat X(4, 4, CV_64F), Y(4, 1, CV_64F), Z(4, 1, CV_64F);
  X.db(0, 0) = shape.db(42, 0);
  X.db(0, 1) = shape.db(42 + n, 0);
  X.db(0, 2) = 1;
  X.db(0, 3) = 0;
  X.db(1, 0) = shape.db(42 + n, 0);
  X.db(1, 1) = -shape.db(42, 0);
  X.db(1, 2) = 0;
  X.db(1, 3) = 1;
  X.db(2, 0) = shape.db(45, 0);
  X.db(2, 1) = shape.db(45 + n, 0);
  X.db(2, 2) = 1;
  X.db(2, 3) = 0;
  X.db(3, 0) = shape.db(45 + n, 0);
  X.db(3, 1) = -shape.db(45, 0);
  X.db(3, 2) = 0;
  X.db(3, 3) = 1;
  Y.db(0, 0) = ref.db(42, 0);
  Y.db(1, 0) = ref.db(42 + n, 0);
  Y.db(2, 0) = ref.db(45, 0);
  Y.db(3, 0) = ref.db(45 + n, 0);
  cv::solve(X, Y, Z, cv::DECOMP_SVD);
  double a = Z.db(0, 0), b = Z.db(1, 0), x = Z.db(2, 0), y = Z.db(3, 0);
  double vx = a * p.x + b * p.y + x, vy = a * p.y - b * p.x + y;
  cv::Point P(vx, vy);
...
//=============================================================================
void myAvatar::WarpBackPupils(cv::Point &pl,
                              cv::Point &pr,
                              cv::Mat &shape,
                              cv::Mat &ref)
{
  pl = this->WarpBackLeftPupil(pl, shape, ref);
  pr = this->WarpBackRightPupil(pr, shape, ref);
  return;
}
//==============================================================================
//=============================================================================
void myAvatar::AddAvatar(cv::Mat &image, cv::Mat &points, cv::Mat &eyes)
{
  ...
  */
  ...
  {
    cv::Mat x = expr.col(j), y = X[j](cv::Rect(0, 0, 1, 2 * n));
    y.copyTo(x);
  }
  //get shape and pose
  ...
  //crop appearance
  cv::Mat gray, grayTex, crop;
  cv::cvtColor(image, gray, CV_BGR2GRAY);
  _warp.Crop(gray, crop, points);
  _warp.Vectorize(crop, grayTex);
  ...
  for (int j = 0; j < 3; j++)
  {
    ...
    _warp.Crop(rgb[j], crop_rgb[j], points);
    _warp.Vectorize(crop_rgb[j], t);
  }
  ...
  */
  ...
  double a, b, tx, ty;
  FACETRACKER::CalcSimT(S, points, a, b, tx, ty);
  ...
  cv::Mat img_rgb;
  cv::merge(crop_rgb, img_rgb);
  _images.push_back(img_rgb);
  ...
  vector<cv::Mat> p(expr.cols);
  shape.copyTo(_pdm._M);
  ...
  if (eyes.rows == 8)
  { //defined pupils
    cv::Mat lpupil, rpupil;
    cv::Scalar lscelera, rscelera;
    double lrad = 0, rrad = 0;
    _gpdm.CalcParams(points, plocal, pglobl);
    ...
    llpupil.scelera = lscelera;
    rrpupil.scelera = rscelera;
    llpupil.image.resize(3);
    cv::split(lpupil, llpupil.image);
    rrpupil.image.resize(3);
    cv::split(rpupil, rrpupil.image);
    _lpupil.push_back(llpupil);
    _rpupil.push_back(rrpupil);
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      cv::Mat t = textr(cv::Rect(0, j * _warp._nPix, 1, _warp._nPix));
      cv::Mat crop;
      _warp.UnVectorize(t, crop);
      _warp.Draw(crop, rgb[j], ds);
    }
    cv::Mat lpt;
    this->GetIdxPts(shape, _lpupil[_idx].idx, lpt, true);
    cv::Mat rpt;
    this->GetIdxPts(shape, _rpupil[_idx].idx, rpt, true);
    ...
    while (0)
    {
      ...
      if (c == 27)
        ...
        _lpupil[_idx].py -= 1; //left up
      else if (c == int('d'))
        _lpupil[_idx].px += 1; //left right
      else if (c == int('x'))
        _lpupil[_idx].py += 1; //left down
      else if (c == int('a'))
        _lpupil[_idx].px -= 1; //left left
      else if (c == int('i'))
        _rpupil[_idx].py -= 1; //right up
      else if (c == int('l'))
        _rpupil[_idx].px += 1; //right right
      else if (c == int('m'))
        _rpupil[_idx].py += 1; //right down
      else if (c == int('j'))
        _rpupil[_idx].px -= 1; //right left
      else if (c == int('b'))
      {
      ...
      { //change pupil appearance to standard
        cv::Mat limg;
        cv::merge(_lpupil[_idx].image, limg);
        limg = cv::Scalar(0, 0, 0);
        cv::Mat rimg;
        cv::merge(_rpupil[_idx].image, rimg);
        rimg = cv::Scalar(0, 0, 0);
        cv::split(limg, _lpupil[_idx].image);
        cv::split(rimg, _rpupil[_idx].image);
      }
    }
    _lpupil[_idx].px += rect.x - 1;
    _lpupil[_idx].py += rect.y - 1;
    _rpupil[_idx].px += rect.x - 1;
    _rpupil[_idx].py += rect.y - 1;
    //make spearate function!
    //***********************************************************************
  }
  else
  { //undefined pupils
    ...
    llpupil.image.resize(3);
    cv::split(limg, llpupil.image);
    rrpupil.image.resize(3);
    cv::split(rimg, rrpupil.image);
    _lpupil.push_back(llpupil);
    _rpupil.push_back(rrpupil);
    ...
    shape = S;
    int n = _pdm.nPoints();
    cv::Rect rect = this->GetBoundingBox(shape);
    shape = points.clone();
    for (int j = 0; j < n; j++)
    {
      shape.db(j, 0) -= rect.x - 1;
      shape.db(j + n, 0) -= rect.y - 1;
    }
    cv::Mat lpt;
    this->GetIdxPts(shape, _lpupil[_idx].idx, lpt, true);
    cv::Mat rpt;
                       ...
                       const int d, const int size)
{
  ...
  for (int i = 0; i < 6; i++)
  {
    lpt.db(i, 0) = pt.db(36 + i, 0);
    lpt.db(i + 6, 0) = pt.db(36 + i + 66, 0);
    rpt.db(i, 0) = pt.db(42 + i, 0);
    rpt.db(i + 6, 0) = pt.db(42 + i + 66, 0);
  }
  ...
  //draw pupils
  leye = cv::Mat::zeros(size, size, CV_8UC3);
  leye = lbck;
  reye = cv::Mat::zeros(size, size, CV_8UC3);
  reye = rbck;
  ...
  for (int y = 0; y < size; y++)
  {
    for (int x = 0; x < size; x++)
    {
      ...
      if (v < (size - 1) / 2)
      {
        ...
        {
          for (int i = 0; i < 3; i++)
            lrgb[i].at<uchar>(y, x) = Tl.db(vi + d * i, 0);
        }
        if (FACETRACKER::isWithinTri(xr, yr, tri, rpt) >= 0)
        {
          for (int i = 0; i < 3; i++)
        ...
        {
          for (int i = 0; i < 3; i++)
            rrgb[i].at<uchar>(y, x) = Tr.db(vi + d * i, 0);
        }
      }
    }
  }
...
//==============================================================================
void myAvatarParams::Save(const char *fname, bool binary)
{
  if (!binary)
  {
    ofstream file;
  ...
  {
    ofstream s(fname, std::ios::binary);
    assert(s.is_open());
...
//==============================================================================
void myAvatarParams::Load(const char *fname, bool binary)
{
  if (!binary)
  {
    ifstream file(fname);
    ...
    file >> t;
    animate_rigid = bool(t);
    file >> t;
    animate_exprs = bool(t);
    file >> t;
    animate_textr = bool(t);
    file >> t;
    animate_eyes = bool(t);
    file >> t;
    avatar_shape = bool(t);
  ...
  {
    ifstream s(fname, std::ios::binary);
    assert(s.is_open());
    int t;
...
{
  double v = cv::norm(x1, x2);
  return exp(-(v * v) / sigma);
}
//=============================================================================
//=============================================================================
//=============================================================================
...
{
  _pdm.CalcParams(s2D, plocal_, pglobl_);
  _pdm.CalcShape3D(shape_, plocal_);
  FACETRACKER::Euler2Rot(R_, pglobl_.db(1, 0), pglobl_.db(2, 0), pglobl_.db(3, 0));
  R_ *= pglobl_.db(0, 0);
  cv::invert(R_, Ri_, cv::DECOMP_SVD);
  cv::Mat x(3, 1, CV_64F), y(3, 1, CV_64F);
  int n = _pdm.nPoints();
  for (int j = 0; j < n; j++)
  {
