...
static bool file_exists_and_is_readable_p(const std::string &filename)
{
    std::ifstream stream(filename.c_str());
    return stream.good();
}

static int how_many_screen_refreshes_have_occurred(int64 now_time, int64 last, int fps)
{
...
static int how_many_frames_have_been_dropped(int64 now_time, int64 last, int fps)
{
...
void draw_health(cv::Mat &I, int health)
{
    ...
    cv::Mat im = I(cv::Rect(h / 2 + 3, h / 2 + 3, v * (w - 5), h - 5));
    im = CV_RGB(0, 0, 255);
    char str[256];
    sprintf(str, "%d%%", (int)(100 * v + 0.5));
    cv::putText(I, str, cv::Point(w + h / 2 + w / 20, h + h / 2),
                CV_FONT_HERSHEY_SIMPLEX, h * 0.045, CV_RGB(0, 0, 255), 2);
    return;
}
//==============================================================================
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        initialise_user_on_first_frame = true;
        //hide_avatar_thumbnail = options.isPresent("hide-avatar-thumbnail");
        hide_avatar_thumbnail = true;
    ...
    bool init = false;
    cv::Mat thumb = avatar->Thumbnail(0);
    ...
    while (1)
    {
        ...
        if (fname)
        {
            ...
            if (((1.0 - frac) < 1.0e-5) || (im.rows == 0) || (im.cols == 0))
                break;
        }
        else
            cv::flip(im, im, 1);
        ...
        if (draw.rows != im.rows)
            draw.create(im.rows, im.cols, CV_8UC3);
        draw = cv::Scalar(0);
